...
\begin{figure}
  ...
  \begin{tikzpicture}[>=latex]
    ...
    \node [block] (initBlobStore) {initBlobStore};
    \node [block, right =1.5cm of initBlobStore] (createBlob) {createBlob};
    \node [block, right =1.5cm of createBlob] (writePartial) {writePartial};
    \node [block, right =1.5cm of writePartial] (finalizeWrite) {finalizeWrite};
    \node [block, below =2.5cm of finalizeWrite] (initRead) {initRead};
    \node [block, above left =0.5cm and 1.5cm of initRead] (readPartial) {readPartial};
    \node [block, below left =0.5cm and 1.5cm of initRead] (skipBytes) {skipBytes};
    \node [block, below left =0.5cm and 1.5cm of readPartial] (finalizeRead) {finalizeRead};

    \path[draw,->] (initBlobStore) edge (createBlob)
                   (createBlob) edge (writePartial)
                   (writePartial) edge [loop above] (writePartial)
                   (writePartial) edge (finalizeWrite)
                   (finalizeWrite) edge (initRead)
                   (initRead) edge (readPartial)
                   (initRead) edge (skipBytes)
                   (readPartial) edge [loop above] (readPartial)
                   (skipBytes) edge [bend right] (readPartial)
                   (readPartial) edge [bend right] (skipBytes)
                   (readPartial) edge (finalizeRead)
                   ;
  \end{tikzpicture}
\end{figure}
...
The \texttt{createBlob} creates an empty file in the \textit{tmp} folder. This method will never fail, as we ensure that the \textit{tmp} folder is always present.
